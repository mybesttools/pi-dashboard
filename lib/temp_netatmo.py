...
class ClientAuth:
    ...
    def __init__(self, clientId, clientSecret, username, password, scope="read_station"):
        post_params = {
            "grant_type": "password",
            "client_id": clientId,
        ...
        resp = postRequest(_AUTH_REQ, post_params)
...
class WeatherStationData:
    ...
    def getMeasure(self, device_id, scale, mtype, module_id=None, date_begin=None, date_end=None, limit=None,
...
if __name__ == "__main__":
    ...
    if not _CLIENT_ID or not _CLIENT_SECRET or not _USERNAME or not _PASSWORD:
        stderr.write("Library source missing identification arguments to check temp_netatmo.py (user/password/etc...)")
        exit(1)
    ...
    try:
        ...
        if stdout.isatty():
            print("temp_netatmo.py : warning, no weather station available for testing")
    else:
    ...
    try:
        ...
        if stdout.isatty():
            print("temp_netatmo.py : warning, no camera available for testing")

    # If we reach this line, all is OK

    # If launched interactively, display OK message
    if stdout.isatty():
        print("temp_netatmo.py : OK")
