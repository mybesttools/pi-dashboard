...
class Plugin(BasePlugin):
    ...
    def _iter_calendars(self):
        for i in range(6):
            baseurl = self.pi_dash.config.get(self.namespace, 'cal%s' % i)
            url = Plugin.build_url(baseurl)
            color = self.pi_dash.config.get(self.namespace, 'color%s' % i)
...
class Config(BaseConfig):
    TEMPLATE = os.path.join(PLUGINDIR, 'gcal_config.html')
