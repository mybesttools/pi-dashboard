# -*- coding: utf-8 -*-
"""
PiDashboard Configuration
"""
import json, keyring, os, shlex
from lib import APPNAME, CONFIGPATH, SHAREDIR
...
class PiConfig(PiWidget):
    ...
    def _init_window(self):
        ...
        self.setWindowTitle('Pi Dashboard Preferences')
        self.setWindowModality(Qt.ApplicationModal)
        ...
        for module in sorted(self.pi_dash.modules.values(), key=self._sortKey):
    ...
    def _sortKey(self, module):
        ...
        return '__pi_dash' if name == 'pi-dashboard' else name
    ...
    def _init_datatable(self):
        ...
        # Build the datatable
        self.datatable_wrap = PiWidget(template, self)
    ...
    def _init_pconfigs(self):
        pconfigs = {}
        for module in filter(lambda m: getattr(m, 'Config', None), self.pi_dash.modules.values()):
            pconfig = module.Config(self.pi_dash, self)
    ...
    def save(self):
        ...
        self.set('pi-dashboard', 'positions', [w.position() for w in self.pi_dash.widgets])
    ...
    def load_tab_data(self, refresh=False):
        namespace = self.manifest.list.currentItem().data(NAMESPACE_ROLE)
        data = self.pi_dash.data.get(namespace, {})
